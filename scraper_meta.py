...
BQ_ENABLE       = os.getenv("BQ_ENABLE", "false").lower() == "true"  # default vypnuté
...
def rand_headers(extra=None, referer=None):
    ...
    if referer:
        ...
        h["Referer"] = "https://www.nehnutelnosti.sk/"
...
def get(session: requests.Session, url: str, extra_headers=None, referer=None) -> requests.Response:
    """Vlastný retry loop s backoff + detailný log (vracia poslednú odpoveď)."""
    tries = 6
    delay = 0.9
    last = None
    for attempt in range(1, tries+1):
        hdr = rand_headers(extra_headers, referer=referer)
        t0 = time.perf_counter()
        try:
            r = session.get(url, headers=hdr, timeout=REQ_TIMEOUT, allow_redirects=True)
        except Exception as e:
            dt = time.perf_counter() - t0
            print(f"[HTTP] EXC {attempt}/{tries} {url} after {dt:.2f}s -> {e}", flush=True)
            if attempt == tries:
                raise
            time.sleep(delay + random.uniform(0,0.6)); delay *= 1.6
            continue
        dt = time.perf_counter() - t0
...
def save_debug(html: str, name: str):
    ...
    try:
        with open(path, "w", encoding="utf-8") as f: f.write(html or "")
        if VERBOSE: print(f"[DEBUG] uložené HTML: {path}", flush=True)
    except Exception as e:
        print(f"[DEBUG] save fail {name}: {e}", flush=True)
    return path
...
def fetch_html_with_fallback(url: str, session: requests.Session, referer: str | None = None, list_html: str | None = None) -> str:
    """Primárne requests; pri 500/429/403/prázdnom tele -> Playwright (list→klik→detail)."""
    r = get(session, url, referer=referer)
                ...
                try:
                    ...
                    try:
                        if list_html:
                            ...
                            if m:
                                pid = m.group(1)
                                a = soup.find("a", href=re.compile(re.escape(pid)))
                                if a and a.get("href"):
                                    sel_href = a.get("href")
                    except Exception:
                        pass
...
def parse_detail(session: requests.Session, url: str, seq_in_run: int, type_hint: str = None, referer: str | None = None, list_html: str | None = None) -> dict:
    html = fetch_html_with_fallback(url, session, referer=referer, list_html=list_html)
    ...
    if price_eur is None and not price_note:
        ...
        save_debug(html, os.path.basename(dbg))
...
def crawl_to_excel(base_url: str = BASE_URL, max_pages: int = MAX_PAGES,
    ...
    for page in range(1, max_pages + 1):
        ...
        r = get(s, page_url, referer=prev_list_url or base_url)
        ...
        for idx, url in enumerate(links, start=1):
            ...
            try:
                rec = parse_detail(
                    s, url, seq_in_run=idx, type_hint=type_hint,
                    referer=page_url, list_html=html
                )
            except Exception as e:
                if VERBOSE:
        ...
        # update referera pre ďalší cyklus stránok
        prev_list_url = page_url
